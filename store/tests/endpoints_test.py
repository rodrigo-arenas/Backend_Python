...
def test_post_user():
    ...
    user_dict = {'name': 'user1', 'password': 'secret', 'mail': 'mymail@mail.com', 'role': 'admin'}
    response = client.post('/v1/user', json=user_dict, headers=auth_header)
    assert response.status_code == 201
    assert check_personnel_mail('user1', 'mymail@mail.com') == True
...
def test_post_user_wrong_email():
    ...
    assert check_personnel_mail('user1', 'mail@mail.com') == False
